...
from transformers import PretrainedConfig, PreTrainedModel, AutoModel, AutoProcessor, AutoModelForCausalLM, \
    AutoTokenizer
...
class VLMConfig(PretrainedConfig):
    ...
    def __init__(self, llm_model_path="Qwen/Qwen2.5-0.5B-Instruct",
                 vision_model_path="google/siglip-so400m-patch14-384", freeze_vision_model=True, image_pad_num=49,
                 **kwargs):
...
class VLM(PreTrainedModel):
    ...
    def __init__(self, config: VLMConfig, *inputs, **kwargs):
        super().__init__(config, *inputs, **kwargs)
        self.config = config
        self.vision_model = AutoModel.from_pretrained(self.config.vision_model_path)
        self.processer = AutoProcessor.from_pretrained(self.config.vision_model_path)
    ...
    def forward(self, input_ids, labels, pixel_values, attention_mask=None):
        ...
        image_embeds = image_embeds.view(b, -1, d * 4)  # (b,196,d) --> (b,49,d*4)s
        ...
        if labels is not None:
            loss_fct = nn.CrossEntropyLoss(ignore_inedex=self.tokenizer.pad_token_id)
